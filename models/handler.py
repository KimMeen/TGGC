    ...
    with torch.no_grad():
        for i, (inputs, target) in enumerate(dataloader):
            ...
            while step < horizon:
                ...
                forecast_result, a, _ = model(inputs)
                ...
                if window_size >= horizon:
                    ...
                    inputs = forecast_result.clone()[:, :window_size, :]
...
def validate(model, dataloader, device, normalize_method, statistic,
    ...
    return dict(mae=score[1], mae_node=score_by_node[1], mape=score[0], mape_node=score_by_node[0],
                rmse=score[2], rmse_node=score_by_node[2], score_norm=score_norm[1])


def train(train_data, valid_data, r, args, result_file):
    node_cnt = train_data.shape[1]
    model = Model(node_cnt, args.layers, args.window_size, args.multi_layer, order=args.order, non_linear=args.non_linear, dropout_rate=args.dropout_rate, leaky_rate=args.leakyrelu_rate, horizon=args.horizon, Fouropt=args.Fouropt, modes=args.modes, activation=args.activation, coe_a=args.coe_a, coe_b=args.coe_b, gconv=args.gconv, attention_set=args.attention_set, device=args.device)
    ...
    back_loss =  nn.MSELoss(reduction='mean').to(args.device)
    total_params = 0
    ...
    for epoch in range(args.epoch):
        ...
        for i, (inputs, target) in enumerate(train_loader):
            ...
            model.zero_grad()
            forecast, _, x_b, = model(inputs)
            loss = forecast_loss(forecast, target) + back_loss(x_b, inputs)
            cnt += 1
            loss.backward()
            my_optim.step()
        ...
        if (epoch + 1) % args.validate_freq == 0:
            ...
            valid_r.append(performance_metrics['score_norm'])
